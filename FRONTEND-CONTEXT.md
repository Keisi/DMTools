...
- Backend (`C:\Users\keisi\source\repos\Personal\DMTool`) IS its own git repo
  ...
  #19–#32 all shipped + consumed — #30 Extra Attack + advantage, **#31 per-campaign
  character state + DM inspiration** → `campaignCharacterState` +
  `components/CampaignCharacterPanel.tsx`, **#32 resource + exhaustion setters** →
  panel resource pips + exhaustion stepper now interactive; committed `0793aae` + `006235d`
  **not yet pushed**; trust the newest INCOMING entry + `CLAUDE.md` over this line).
  Prod Azure backend not yet past mig 069 → #29/#30/#31/#32 local-only until deploy.
...
> — this section lags. As of 2026-06-14 the #31 follow-ups (campaign-resource-set +
> campaign-exhaustion-set) shipped as INCOMING #32 and are **consumed** — the panel's
> resource pips + exhaustion stepper are now interactive. The entries below are older
> and may already be partly addressed; defer to `CLAUDE.md`.
